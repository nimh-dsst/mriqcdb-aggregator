...
from mriqc_aggregator.loading import load_dump, load_raw_run
    ...
    payload = [
        {
            ...
            "_updated": {"$date": "2017-06-08T05:54:47.000Z"},
            "cjv": 0.1,
            "cnr": 0.2,
            "efc": 0.3,
            "fber": 0.4,
...
def test_load_dump_is_idempotent(tmp_path: Path, postgres_database_url: str) -> None:
    ...
    assert second.per_modality["T1w"].inserted_count == 0
    assert second.per_modality["T1w"].updated_count == 1
    assert record.source_api_id == "abc123def456abc123def456"
